...
<main> <!--Левое меню -->
    <div class="container hero-container">
        <ul class="hero-list">
            ...
            <li class="hero-item__tasks _active-sos">
                <a class="tasks-btn btn _active-btn" href="/tasks"><img src="/img/svg/tasks.svg" class="tasks-img hero-img" alt="">ЗАДАЧИ</a>
            </li>
        ...
        <div class="work-area _invis">
            <ul class="tasks-list">
                <li class="tasks-item example-dropzone" ondrop="drop(event)" ondragover="allowDrop(event)">
                    <h1>Просроченнные задания</h1>
                    @if($executed->count() != 0)

                    <p>{{$executed->count()}}</p>
                    @foreach($executed as $item)
                    <div class="line-ver"></div>
                    <div class="event-box example-draggable" id="{{$item->id}}" draggable="true" ondragstart="drag(event)">
                        <div class="event-box__date">
                            <div class="_blue">{{$item->deadline}}</div><div class="_red"></div>
                        </div>
                        <h3>Ответственный:{{$item->user->name}}</h3>
                        <div>{{$item->title}}</div>
                    </div>
                        @endforeach
                    @else
                ...
                <li class="tasks-item example-dropzone" ondrop="dropDay(event)" ondragover="allowDrop(event)">
                    ...
                    @if($day->count() != 0)
                        ...
                        @foreach($day as $item)
                            <div class="line-ver"></div>
                            <div class="event-box" draggable="true"  id="{{$item->id}}" ondragstart="drag(event)">
                                <div class="event-box__date">
                                    <div class="_blue">{{$item->deadline}}</div><div class="_red"></div>
                                </div>
                                <h3>Ответственный:{{$item->user->name}}</h3>
                ...
                <li class="tasks-item example-dropzone" ondrop="dropNext(event)" ondragover="allowDrop(event)">
...
<script>
    ...
    function dropDay(ev) {
        ...
        $.ajax({
            url: 'http://127.0.0.1:8000/api/tasksUpdate',         /* Куда пойдет запрос */
